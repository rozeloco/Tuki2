...
function addMetadata(packname, author) {	
	if (!packname) packname = ''; if (!author) author = '@tkkpost on ig';	
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Hey @${num.split('@')[0]} bienvenido a *${mdata.subject}* recuerda leer las reglas.`
